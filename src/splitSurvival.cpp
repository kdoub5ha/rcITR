...
// [[Rcpp::export]] 
Rcpp::List splitSurvival(NumericVector zcut,
  ...
  NumericVector outputAE(zlength);
  // =========================================================
  ...
  for(int i = 0; i < zlength; i++){
    if((condA[i] >= nodeSize) &&
      ...
      // If conditions are passed for a given split calculate the value;
      for(int j = 0; j < ylength; j++){
        if(inNode[j]){
          if(isCtg){
            outputL[i] += y[j]*(trt[j]*status[j]*(zcutCat(j,i)) / (divisor*prtx[j]*kmCens[j]) + 
                    (1 - trt[j])*status[j]*(1-zcutCat(j,i)) / (divisor*(1 - prtx[j])*kmCens[j]));
            outputR[i] += y[j]*(trt[j]*status[j]*(1-zcutCat(j,i)) / (divisor*prtx[j]*kmCens[j]) + 
                    (1 - trt[j])*status[j]*(zcutCat(j,i)) / (divisor*(1 - prtx[j])*kmCens[j]));
          } else{
            outputL[i] += y[j]*(trt[j]*status[j]*(x[j] <= zcut[i]) / (divisor*prtx[j]*kmCens[j]) + 
              (1 - trt[j])*status[j]*(x[j] > zcut[i]) / (divisor*(1 - prtx[j])*kmCens[j]));
            outputR[i] += y[j]*(trt[j]*status[j]*(x[j] > zcut[i]) / (divisor*prtx[j]*kmCens[j]) + 
              (1 - trt[j])*status[j]*(x[j] <= zcut[i]) / (divisor*(1 - prtx[j])*kmCens[j]));
          }
        } else{
          // If the other nodes are to be used in the value calculation then do so;
          if(useOtherNodes){
            outputL[i] += y[j]*(trt[j]*status[j]*(trtNew[j]) / (divisor*prtx[j]*kmCens[j]) + 
                    (1 - trt[j])*status[j]*(1 - trtNew[j]) / (divisor*(1 - prtx[j])*kmCens[j]));
            outputR[i] += y[j]*(trt[j]*status[j]*(trtNew[j]) / (divisor*prtx[j]*kmCens[j]) + 
                    (1 - trt[j])*status[j]*(1 - trtNew[j]) / (divisor*(1 - prtx[j])*kmCens[j]));
          }
        }
      }
